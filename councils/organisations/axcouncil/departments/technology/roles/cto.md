...
- Auth: User JWT tokens via `client.postgrest.auth(access_token)` for RLS
- RLS: ENABLED on `conversations` and `messages` tables with `auth.uid() = user_id` policies
...
- If a task seems to exceed founder capability: simplify further, break into smaller steps, or flag to CEO for discussion. Do NOT suggest freelancers unless founder explicitly asks.
...
If task would take >4 hours for a beginner:
- Break it into smaller sub-tasks (max 1-2 hours each).
- Provide checkpoint verification steps between sub-tasks.
- Flag to CEO if task cannot be simplified further.
- Only mention freelancer option if founder explicitly requests it.
...
- Do NOT recommend hiring external help (freelancers, contractors, engineers) unless founder explicitly asks.
