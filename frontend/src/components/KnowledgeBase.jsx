...
// Skeleton for loading state (declared outside component to avoid re-creation)
  ...
  return (
    <div className="kb-skeleton">
      ...
      <div className="kb-category-tabs">
        {[1, 2, 3, 4, 5].map(i => (
          <Skeleton key={i} width={80} height={36} className="skeleton-badge" />
        ))}
      </div>

      {/* Search row skeleton */}
      <div className="kb-search-row" style={{ marginTop: 16 }}>
        <Skeleton height={40} style={{ flex: 1 }} />
        <Skeleton width={140} height={40} />
      </div>

      {/* Entry list skeleton */}
      <div className="kb-entries" style={{ marginTop: 24 }}>
        {[1, 2, 3, 4].map(i => (
          <div key={i} className="kb-entry-skeleton">
            <div className="kb-entry-header" style={{ pointerEvents: 'none' }}>
...
export default function KnowledgeBase({ companyId, conversationFilter = null, onClose }) {
  ...
  // Fetch entries when filters change (data fetching on mount/filter change is valid)
  useEffect(() => {
    if (companyId) {
  ...
  return (
    ...
    >
      ...
      {/* Edit Modal - using nested AppModal */}
      <AppModal
        isOpen={!!editingEntry}
