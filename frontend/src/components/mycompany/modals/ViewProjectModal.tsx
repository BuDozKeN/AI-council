...
export function ViewProjectModal({
  ...
  companyId,
  departments = [],
  initialExpandedDecisionId = null,
  ...
  const [activeTab, setActiveTab] = useState<TabType>(
    initialExpandedDecisionId ? 'decisions' : 'context'
  );
  ...
  const handleExpandDecision = async (decisionId: string) => {
    ...
    const needsRegeneration =
      decision && decision.question && isGarbageSummary(decision.content_summary);
  ...
  const groupedDecisions = useMemo((): DecisionGroup[] => {
    ...
    const sorted = [...projectDecisions].sort(
  ...
  useEffect(() => {
    ...
    if (companyId && project.id && !isEditing && syncedProjectId.current !== project.id) {
      ...
      api
        .syncProjectDepartments(companyId, project.id)
        .then((syncResult) => {
  ...
  useEffect(() => {
    if (activeTab === 'decisions' && companyId && project.id && projectDecisions.length === 0) {
      ...
      api
        .getProjectDecisions(companyId, project.id)
        .then((data) => {
  ...
  const getStatusColor = (status: ProjectStatus): { bg: string; text: string } => {
    switch (status) {
      case 'active':
        return { bg: 'var(--color-blue-100)', text: 'var(--color-blue-700)' };
      case 'completed':
        return { bg: 'var(--color-green-100)', text: 'var(--color-green-700)' };
      case 'archived':
    ...
    >
      ...
      <div className="mc-modal-body" ref={modalBodyRef}>
        ...
        <div className="mc-project-meta-row">
          ...
          {!isNew && !isEditing && (project.created_at || project.updated_at) && (
            <div className="mc-project-timestamps">
              {project.created_at && (
                <span className="mc-timestamp">{formatDateShort(project.created_at)}</span>
              )}
        ...
        {isEditing ? (
          ...
          project.description && (
            <p className="mc-project-description">{truncateText(project.description, 200)}</p>
          )
        )}
          ...
          <div className="mc-project-tabs">
            ...
            >
              <svg className="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor">
                <path
                  fillRule="evenodd"
                  d="M4 4a2 2 0 012-2h8a2 2 0 012 2v12a1 1 0 110 2H4a1 1 0 010-2zm3 1h6v4H7V5zm6 6H7v2h6v-2z"
                  clipRule="evenodd"
                />
              </svg>
              {t('modals.contextTab')}
                                ...
                                {(decision.department_names || []).map((deptName, idx) => {
                                  ...
                                  return (
                                    <span
                                      ...
                                      style={{
                                        background:
                                          getDeptColor(deptId)?.bg || 'var(--color-gray-100)',
                                        color:
                                          getDeptColor(deptId)?.text || 'var(--color-gray-700)',
                                        borderColor:
                                          getDeptColor(deptId)?.border || 'var(--color-gray-200)',
                                      }}
                                    >
                                      {deptName}
                                    </span>
                            ...
                            <div className="mc-timeline-body">
                              ...
                              {generatingSummaryId === decision.id ? (
                                ...
                                <div className="mc-timeline-summary">
                                  <MarkdownViewer
                                    content={decision.content_summary}
                                    skipCleanup={true}
                                  />
                                </div>
                              ) : null}
                              ...
                              {decision.source_conversation_id && onNavigateToConversation && (
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    const inferredIndex =
                                      decision.response_index ?? decisionIndex * 2 + 1;
