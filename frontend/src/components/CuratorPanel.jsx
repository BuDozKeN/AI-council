...
export default function CuratorPanel({
  ...
  onClose,
  initialStatus = null, // Pre-fetched status from parent to avoid loading delay
}) {
  ...
  // Skip loading if we have initialStatus from parent
  const [historyLoading, setHistoryLoading] = useState(!initialStatus);
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        // Fetch both in parallel but handle failures independently
        const [historyResult, lastUpdatedResult] = await Promise.allSettled([
        ...
        if (historyResult.status === 'fulfilled') {
          setCuratorHistory(historyResult.value?.history || []);
        } else {
          console.error('Failed to fetch curator history:', historyResult.reason);
          setCuratorHistory([]);
        }

        // Process last updated result
        if (lastUpdatedResult.status === 'fulfilled') {
          setContextLastUpdated(lastUpdatedResult.value?.last_updated || null);
        } else {
          console.error('Failed to fetch context last updated:', lastUpdatedResult.reason);
          setContextLastUpdated(null);
        }
      } catch (err) {
  ...
  // Format date as relative time (e.g., "2 hours ago", "3 months ago")
  // Uses relative time to avoid US/EU date format confusion
  ...
  const getHistoryStatus = () => {
    if (!curatorHistory || curatorHistory.length === 0) {
      return {
        type: 'never',
        message: 'Curator has never been run on this conversation',
        relativeTime: null,
        savedCount: 0,
        total: 0
      };
    ...
    const total = lastRun.suggestions_count || lastRun.suggestion_count || 0;
    const savedCount = lastRun.accepted_count || 0;
    const relativeTime = formatRelativeTime(lastRunDate);

    // Check if context has been updated since last curator run
    if (contextLastUpdated && lastRunDate) {
      try {
        ...
        if (contextDate > runDate) {
          return {
            type: 'outdated',
            message: `Last run ${relativeTime} - Knowledge base updated since then`,
            relativeTime,
            savedCount,
            total,
            lastRun
    ...
    return {
      type: 'current',
      message: `${savedCount}/${total} saved ${relativeTime}`,
      relativeTime,
      lastRun,
      savedCount,
      total
  ...
  const handleClose = async () => {
    ...
    if (status === 'done' && hasSavedRun) {
      ...
      if (acceptedCount > 0 || rejectedCount > 0) {
        try {
          // Wait for save to complete before closing, so parent can fetch updated status
    ...
    return (
      <div className="curator-panel">
        <div className="curator-header">
          ...
          {/* Show status in header so it's immediately visible */}
          {!historyLoading && historyStatus.type === 'current' && (
            <div className="curator-header-status">
              <span className="header-status-icon">✓</span>
              <span className="header-status-text">{historyStatus.message}</span>
            </div>
          )}
          <button className="curator-close" onClick={handleClose}>×</button>
        </div>
        <div className="curator-content">
          {historyStatus.type === 'never' ? (
            <>
            ...
            <>
              <p className="curator-intro">
                This conversation was last reviewed <strong>{historyStatus.relativeTime || 'previously'}</strong>.
                {historyStatus.savedCount > 0
                  ? ` ${historyStatus.savedCount} insight${historyStatus.savedCount !== 1 ? 's were' : ' was'} saved to your knowledge base.`
                  : ' No insights were saved.'}
              </p>
              <p className="curator-intro-sub">
                Run again to check for new insights if the conversation has continued.
              </p>
            </>
          )}
          <div className="curator-start-actions">
            <button className="curator-start-btn" onClick={startAnalysis} disabled={historyLoading}>
              {historyStatus.type === 'never' ? 'Analyse Conversation' : 'Check for New Insights'}
            </button>
  ...
  return (
    <div className="curator-panel">
      <div className="curator-header">
        ...
        <div className="curator-title">Knowledge Curator</div>
        {totalSuggestions > 0 && (
          <div className="curator-progress-badge">
            <span className="progress-saved">{acceptedCount}</span>
            <span className="progress-separator">/</span>
            <span className="progress-total">{totalSuggestions}</span>
        ...
        {suggestions.length === 0 ? (
          ...
          <>
            <div className="curator-summary">
              <p>{summary}</p>
              <div className="curator-progress-detail">
                {acceptedCount > 0 && (
                  <span className="progress-item saved">✓ {acceptedCount} saved</span>
                )}
                {rejectedCount > 0 && (
                  <span className="progress-item rejected">× {rejectedCount} skipped</span>
                )}
                {pendingSuggestions.length > 0 && (
                  <span className="progress-item pending">○ {pendingSuggestions.length} remaining</span>
                )}
              </div>
            </div>
                      ...
                      <>
                        ...
                        {suggestion.last_updated && (
                          <div className="file-info">
                            Knowledge base last updated: {formatRelativeTime(suggestion.last_updated)}
                          </div>
                        )}
