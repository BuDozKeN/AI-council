...
export function getModelPersona(modelId: string | null | undefined): ModelPersonaWithLabel {
  if (!modelId)
    return { ...DEFAULT_PERSONA, providerLabel: PROVIDER_LABELS[DEFAULT_PERSONA.provider] };
  ...
  if (MODEL_PERSONAS[modelId]) {
    ...
    return {
      ...persona,
      providerLabel: PROVIDER_LABELS[persona.provider] ?? persona.provider.toUpperCase(),
    };
  }
