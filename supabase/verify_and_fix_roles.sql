...
-- https://supabase.com/dashboard/project/gkwpmxspxolxdlvnfnpa/sql/new
...
SELECT
    ...
    name,
    slug,
    '' as system_prompt_preview
FROM companies
WHERE slug = 'axcouncil';
...
SELECT
    ...
    r.name,
    r.slug,
    LEFT(COALESCE(r.system_prompt, ''), 50) as system_prompt_preview
FROM roles r
JOIN companies c ON r.company_id = c.id
WHERE c.slug = 'axcouncil';

-- STEP 2: If system_prompt is empty, run these UPDATEs:
...
-- Update Head of AI People & Culture
UPDATE roles
...
- Significant organizational restructuring is proposed',
    updated_at = NOW()
WHERE slug = 'ai-people-culture'
AND company_id = (SELECT id FROM companies WHERE slug = 'axcouncil');

-- Update AI UX/UI Designer
UPDATE roles
...
SELECT
    r.name,
    r.slug,
    LENGTH(COALESCE(r.system_prompt, '')) as prompt_length,
    LEFT(COALESCE(r.system_prompt, 'EMPTY'), 100) as preview
FROM roles r
JOIN companies c ON r.company_id = c.id
WHERE c.slug = 'axcouncil'
ORDER BY r.name;
