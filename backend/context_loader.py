...
def get_system_prompt_with_context(
    ...
    REFACTORED: Complexity reduced from F (56) to C (~15) by extracting helper functions.
    ...
    # Import helper functions from refactored module
    ...
    system_prompt = _inject_project_context(
        system_prompt, project_id, access_token, storage, truncate_to_limit, MAX_SECTION_CHARS
    )

    # 8. Inject department contexts if selected
    system_prompt = _inject_department_contexts(
        system_prompt, all_department_ids, get_supabase_service,
        load_department_context_from_db, get_department_roles, is_valid_uuid
    )

    # 9. Inject explicitly selected playbooks
    system_prompt = _inject_playbooks(
        system_prompt, playbook_ids, get_supabase_service, truncate_to_limit, MAX_SECTION_CHARS
    )

    # 10. Add response guidance and role-specific instructions
    system_prompt = _add_response_guidance(
        system_prompt, role_infos, all_role_ids, all_department_ids, is_valid_uuid
    )
